...
public function phantrangsearchType_id($keyword,$type_id,$start_from)
{
     $sql = self::$connection->prepare("SELECT * FROM products WHERE 
     `description` LIKE ? AND `type_id` = ? LIMIT ?,4 ");
     $keyword = "%$keyword%";
     $sql->bind_param("sii",$keyword,$type_id,$start_from);
...
//----------------------khi có type_id-------------------------------------
          ...
          public function phantrang($type_id,$manu_id,$start_from){
               $sql = self::$connection->prepare( "SELECT * FROM `products` WHERE `type_id` = ?  AND `manu_id` = ? LIMIT ?,9");
               $sql->bind_param("iii",$type_id,$manu_id,$start_from);
               $sql -> execute();//return the object
               //chuyen du lieu thanh mang
               $data = array();
               $data = $sql -> get_result()-> fetch_all(MYSQLI_ASSOC);
          ...
          public function phantrangtheoid($type_id,$start_from)
          {
          //lay du lieu
               $sql = self::$connection->prepare("SELECT * FROM products WHERE `type_id` = ?   LIMIT ? ,9  ");
               $sql->bind_param("ii",$type_id,$start_from);
               $sql -> execute();//return the object
               //chuyen du lieu thanh mang
               $data = array();
               $data = $sql -> get_result()-> fetch_all(MYSQLI_ASSOC);
...
//----------------------khi không có type_id-------------------------------------
          ...
          public function phantrangNotType_id($manu_id,$start_from){
               $sql = self::$connection->prepare( "SELECT * FROM `products` WHERE  `manu_id` = ? LIMIT ?,9");
               $sql->bind_param("ii",$manu_id,$start_from);
               $sql -> execute();//return the object
               //chuyen du lieu thanh mang
               $data = array();
               $data = $sql -> get_result()-> fetch_all(MYSQLI_ASSOC);
          ...
          public function phantrangNotType_idtheoid($start_from)
          {
          //lay du lieu
               $sql = self::$connection->prepare("SELECT * FROM products  LIMIT ? ,9  ");
